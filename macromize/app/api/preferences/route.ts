...
export async function GET(request:Request){
 try{const id=guest(request);const row=await database().prepare("SELECT targets, saved, onboarding_completed FROM guest_state WHERE id = ?").bind(id).first<{targets:string;saved:string;onboarding_completed:number}>();
 return Response.json(row?{targets:targetsSchema.parse(JSON.parse(row.targets)),saved:JSON.parse(row.saved),onboardingCompleted:row.onboarding_completed===1}:{targets:defaults,saved:[],onboardingCompleted:false},{headers:headers(id,request)});
...
export async function PUT(request:Request){
 ...
 try{const body=await request.json();const input=z.object({onboardingCompleted:z.boolean().optional(),targets:targetsSchema,saved:z.array(z.string().max(100)).max(200)}).parse(body);
 ...
 await database().prepare("INSERT INTO guest_state (id, targets, saved, onboarding_completed) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET targets=excluded.targets, saved=excluded.saved, onboarding_completed=COALESCE(?, guest_state.onboarding_completed)").bind(id,JSON.stringify(input.targets),JSON.stringify([...new Set(input.saved)]),input.onboardingCompleted===undefined?1:Number(input.onboardingCompleted),input.onboardingCompleted===undefined?null:Number(input.onboardingCompleted)).run();
