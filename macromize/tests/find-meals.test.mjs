...
new Function('require','exports',code)(name=>({'react':React,'react/jsx-runtime':jsx,'./avocado-score':badgeExports,'./meal-map':{__esModule:true,default:()=>null},'../lib/macromize':domain,'../lib/use-macro-editor':macroEditor,'lucide-react':icons}[name]),exports);
...
test('compact recommendations open details and keep save and website actions out of the overview',()=>{
 const html=render([meal]);assert.match(html,/Nutrition unknown/);assert.doesNotMatch(html,/Website unavailable/);assert.doesNotMatch(html,/Get directions/);assert.doesNotMatch(html,/Save options/);assert.match(html,/View details for Fixture meal/);assert.match(html,/No photo/);assert.match(html,/Recommendations/);assert.match(html,/No limit/);assert.doesNotMatch(html,/null kcal/);assert.match(html,/Match score unavailable/);assert.doesNotMatch(html,/Edit macros/);
});
