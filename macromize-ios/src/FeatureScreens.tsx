...
const C = { green: '#073D2B', ink: '#101C21', muted: '#6C786A', line: '#E0E7D9', white: '#FFFFFF', soft: '#EDF2E6' };
...
export function Choice({ values, value, onChange }: { values: string[]; value: string; onChange: (value: string) => void }) { return <View style={f.choices}>{values.map(v => <Pressable key={v} accessibilityRole="radio" accessibilityLabel={v} accessibilityState={{ checked: value === v }} onPress={() => onChange(v)} style={[f.choice, value === v && f.selected]}><Text style={{ color: value === v ? C.white : C.ink, fontSize: 13 }}>{v}</Text></Pressable>)}</View>; }
export function AdvancedTargets({ targets, onChange }: { targets: Targets; onChange: <K extends keyof Targets>(key: K, value: Targets[K]) => void }) {
  ...
  return <View style={f.section}><Pressable accessibilityRole="button" accessibilityState={{ expanded: open }} onPress={() => setOpen(!open)} style={f.between}><Text style={f.heading}>More filters & context</Text><Ionicons name={open ? 'chevron-up' : 'chevron-down'} size={20} color={C.green} /></Pressable>{open && <>
    {numeric('budgetMax', 'Maximum price (€)')}{numeric('carbsMax', 'Carbs maximum (g)')}{numeric('fatMax', 'Fat maximum (g)')}
